...
class Schema {
  ...
  expandProperties() {
    ...
    return resolved.then(properties => {
      return _.extend(properties, this._schema.properties);
    });
  }
    ...
    return Promise.props(_.mapValues(this.properties, prop => {
      ...
      if (this.type) {
        return this.loadType(this.type).then(type => {
          return type.toJSONSchema(context).then(jsonSchema => {
            context.definitions[type.name] = jsonSchema;
            const reference = `#/definitions/${type.name}`;
            return {type: 'object', allOf: [reference, jsonSchema]};
          });
        });
        ...
        return Promise.map(this.extend, schema => {
          ...
          return {type: 'object', allOf: schemas.concat([jsonSchema])};
        });
      }
